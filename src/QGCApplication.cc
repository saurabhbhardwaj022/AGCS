...
void QGCApplication::setLanguage()
{
    ...
    //-- We have specific fonts for Hindi
    if(_locale == QLocale::Hindi) {
        qCDebug(LocalizationLog) << "Loading Hindi fonts" << _locale.name();
        if(QFontDatabase::addApplicationFont(":/fonts/NotoSansDevanagari-Regular") < 0) {
            qCWarning(LocalizationLog) << "Could not load /fonts/NotoSansDevanagari-Regular font";
        }
        if(QFontDatabase::addApplicationFont(":/fonts/NotoSansDevanagari-SemiBold") < 0) {
            qCWarning(LocalizationLog) << "Could not load /fonts/NotoSansDevanagari-SemiBold font";
        }
    }
