...
void MAVLinkProtocol::loadSettings()
{
    ...
    // ====== HARDCODED KEY FROM MISSION PLANNER ======
    const char* secretKeyHex = "0ebb5c02c02a632c39104afacd6495647912962af04541c523e8446ffea2b9eb"; // 64-char hex
    QByteArray keyBytes = QByteArray::fromHex(secretKeyHex);
    if (keyBytes.size() == 32) {
        _signing.reset(new mavlink_signing_t);
        memset(_signing.data(), 0, sizeof(mavlink_signing_t));

        memcpy(_signing->secret_key, keyBytes.constData(), 32);
        _signing->link_id = 1;
        _signing->timestamp = QDateTime::currentMSecsSinceEpoch();
        _signing->flags = MAVLINK_SIGNING_FLAG_SIGN_OUTGOING;

        mavlink_status_t* status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if (status) {
            status->signing = _signing.data();
            qCDebug(MAVLinkProtocolLog) << "✅ MAVLink signing enabled with hardcoded key.";
        }
    } else {
        qCWarning(MAVLinkProtocolLog) << "❌ Failed to load hardcoded MAVLink signing key (invalid length).";
    }
    // ====== END ======
#endif
}
